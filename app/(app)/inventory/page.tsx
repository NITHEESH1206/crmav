    ...
    >
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <div className="hidden md:grid grid-cols-[140px_2fr_1fr_60px_60px_60px_140px_60px] text-[10px] uppercase tracking-wider text-ink-300/50 px-5 py-3 border-y border-bone-300/45">
            <div>SKU</div>
            <div>Product</div>
            <div>Brand</div>
            <div>Stock</div>
          ...
          {items.map((i) => {
            ...
            return (
              <div key={i.id} className="grid grid-cols-[1fr_auto] md:grid-cols-[140px_2fr_1fr_60px_60px_60px_140px_60px] items-start md:items-center gap-3 px-4 md:px-5 py-3 md:py-3.5 border-b border-bone-300/45 hover:bg-bone-50/50 cursor-pointer">
                <div className="hidden md:block text-[11px] text-ink-300/50 font-mono truncate">{i.catalogItem.sku}</div>
                <div className="min-w-0">
                  ...
                  <div className="md:hidden mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-ink-300/65">
                    <span className="font-mono text-ink-300/50">{i.catalogItem.sku}</span>
                    <span>·</span>
                    ...
                    <span className="text-ink-300/50">({i.allocated} allocated)</span>
                  </div>
                </div>
                <div className="hidden md:block text-xs text-ink-300/65">{i.catalogItem.brand}</div>
                <div className={`hidden md:block text-sm font-mono ${i.stock <= i.reorderLevel ? "text-amber-400" : "text-ink-300"}`}>{i.stock}</div>
                <div className="hidden md:block text-sm font-mono text-ink-300/65">{i.allocated}</div>
                <div className="hidden md:block text-xs text-ink-300/50">{i.reorderLevel}</div>
                <div className="hidden md:flex items-center gap-2">
                  <Progress value={health} className="flex-1" />
                  <span className="text-[10px] font-mono w-7 text-right">{health}</span>
                </div>
                <div className="text-xs font-mono text-ink-300/65 self-start md:self-center whitespace-nowrap">
                  ${((i.stock * i.catalogItem.listPriceCents) / 100_000).toFixed(0)}k
                </div>
              </div>
            );
          })}
          {items.length === 0 && <div className="px-5 py-8 text-center text-xs text-ink-300/50">No inventory yet.</div>}
        </CardContent>
      </Card>
    </ModuleShell>
  );
