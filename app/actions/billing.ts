...
import { PLAN_PRICE_CENTS, PLAN_LABEL } from "@/lib/billing/plans";
 ...
 */
export async function createPlanUpgradeLink(input: z.infer<typeof planSchema>) {
  const { plan } = planSchema.parse(input);
  await assertOwner();
  if (plan === "ENTERPRISE") {
  ...
  try {
    const link = await createPaymentLink({
      amountCents,
      currency: "INR",
      description: `ZynexAV ${PLAN_LABEL[plan]} plan — 1 month`,
