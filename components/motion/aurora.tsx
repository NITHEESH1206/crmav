...
export function Aurora({ className = "" }: { className?: string }) {
  return (
    <div aria-hidden className={`pointer-events-none absolute inset-0 overflow-hidden ${className}`}>
      <motion.div
        className="absolute -top-1/2 left-1/4 h-[60rem] w-[60rem] rounded-full bg-aether-500/20 blur-[120px]"
        animate={{ x: [0, 60, -40, 0], y: [0, 30, -20, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-1/4 -right-1/4 h-[50rem] w-[50rem] rounded-full bg-aether-600/15 blur-[120px]"
